"""Offline tests for the regional streaming-availability refresh page."""
...
from media_recommender.web.routes.availability import (
    AVAILABILITY_REFRESH_PATH,
    AvailabilityConfigurationState,
    availability_configuration_state,
    get_availability_configuration_state,
    get_availability_refresh_service,
)
...
_TMDB_BASE_URL_ENV = "MEDIA_RECOMMENDER_TMDB_BASE_URL"
...
_SYNTHETIC_TMDB_URL = "https://tmdb.synthetic.invalid/"
...
_EXPECTED_REPEAT_CALLS = 2


class FakeAvailabilityService:
    """Record availability-only facade calls and return a configured report."""
    ...
    async def refresh_streaming_availability(self, region: str) -> WorkflowReport:
        ...
        :param region: Requested availability region.
...
def _report(
    ...
    """Build a synthetic streaming-availability report.
    ...
    """
    return WorkflowReport(kind=WorkflowKind.STREAMING_AVAILABILITY, status=status, counts=counts, items=items)


def _failure_report(reason: WorkflowFailureReason) -> WorkflowReport:
    """Build a synthetic failed availability report with one safe reason.
...
def _client(
    ...
    *,
    configuration: AvailabilityConfigurationState = AvailabilityConfigurationState.CONFIGURED,
) -> Client:
    """Build a test client with the availability facade and configuration overridden.

    :param service: Synthetic availability facade.
    :param configuration: Synthetic TMDB configuration state.
    :return: Offline HTTPX-compatible test client.
    """
    app = create_app()
    app.dependency_overrides[get_availability_refresh_service] = lambda: service
    app.dependency_overrides[get_availability_configuration_state] = lambda: configuration
    return cast("Client", TestClient(app))


def _client_without_service(
    *,
    configuration: AvailabilityConfigurationState | None = None,
) -> Client:
    """Build a test client without an installed availability refresh service.

    :param configuration: Optional synthetic TMDB configuration state.
...
def _csrf_token(client: Client) -> str:
    """Return the CSRF token rendered by the availability refresh page.

    :param client: Test client carrying a session cookie.
    :return: Hidden form token value.
    """
    response = client.get(AVAILABILITY_REFRESH_PATH)
...
def _post(client: Client, *, token: str | None = None, origin: str | None = None) -> Response:
    """Submit one synthetic availability refresh request.
    ...
    return client.post(AVAILABILITY_REFRESH_PATH, data=data, headers=headers)
...
def test_get_renders_accessible_form_with_navigation_and_csrf_token() -> None:
    ...
    client = _client(FakeAvailabilityService(_report(WorkflowStatus.SUCCESS, WorkflowCounts())))

    response = client.get(AVAILABILITY_REFRESH_PATH)

    assert response.status_code == HTTPStatus.OK
    assert 'href="http://testserver/availability/refresh">Availability refresh</a>' in response.text
...
def test_absent_configuration_disables_control_without_invoking_facade() -> None:
    ...
    service = FakeAvailabilityService(_report(WorkflowStatus.SUCCESS, WorkflowCounts()))
    client = _client(service, configuration=AvailabilityConfigurationState.ABSENT)
    token = _csrf_token(client)

    page = client.get(AVAILABILITY_REFRESH_PATH)
...
def test_invalid_configuration_is_distinct_and_does_not_invoke_facade() -> None:
    ...
    service = FakeAvailabilityService(_report(WorkflowStatus.SUCCESS, WorkflowCounts()))
    client = _client(service, configuration=AvailabilityConfigurationState.INVALID)
    token = _csrf_token(client)
...
def test_success_renders_aggregate_counts_and_removed_entries() -> None:
    """Render supported aggregate counts only for a completed refresh."""
    report = _report(
        WorkflowStatus.SUCCESS,
        WorkflowCounts(succeeded=3, unresolved=2, ambiguous=4, removed=2),
    )
    ...
    assert service.regions == ["CZ"]
    assert "Availability refresh completed." in response.text
...
def test_authentication_failure_is_distinct_and_free_of_raw_reasons() -> None:
    ...
    service = FakeAvailabilityService(_failure_report(WorkflowFailureReason.AUTHENTICATION_FAILURE))
    client = _client(service)
    token = _csrf_token(client)
    ...
    assert "<dt>Removed</dt>" not in response.text
...
def test_generic_failure_is_sanitized() -> None:
    ...
    service = FakeAvailabilityService(_failure_report(WorkflowFailureReason.PROVIDER_FAILURE))
    client = _client(service)
    token = _csrf_token(client)
...
def test_unexpected_exception_is_translated_without_leakage(
    ...
    service = FakeAvailabilityService(
        error=RuntimeError(f"{_SYNTHETIC_TMDB_URL} {_SYNTHETIC_TMDB_TOKEN} {_SYNTHETIC_REASON}")
    )
    client = _client(service)
    token = _csrf_token(client)
    ...
    assert "could not be completed" in response.text
    assert "<dt>Refreshed</dt>" not in response.text
    for value in (_SYNTHETIC_TMDB_URL, _SYNTHETIC_TMDB_TOKEN, _SYNTHETIC_SECRET, _SYNTHETIC_REASON):
        assert value not in response.text
        assert value not in caplog.text


def test_missing_or_incorrect_csrf_token_is_rejected_before_refresh() -> None:
    """Reject submissions without a valid session CSRF token."""
    service = FakeAvailabilityService(_report(WorkflowStatus.SUCCESS, WorkflowCounts(succeeded=1)))
    client = _client(service)
    _csrf_token(client)
...
def test_cross_origin_submission_is_rejected_before_refresh() -> None:
...
def test_repeat_submission_is_a_new_snapshot() -> None:
    """Treat a second explicit submission as a new refresh rather than an automatic retry."""
    service = FakeAvailabilityService(_report(WorkflowStatus.SUCCESS, WorkflowCounts(succeeded=1)))
    client = _client(service)
    token = _csrf_token(client)
    ...
    assert service.calls == _EXPECTED_REPEAT_CALLS
    assert service.regions == ["CZ", "CZ"]
...
def test_configured_request_does_not_leak_synthetic_configuration(
    ...
    """Serve a successful refresh with synthetic secrets configured and leak none of them."""
    monkeypatch.setenv(_SESSION_SECRET_ENV, _SYNTHETIC_SECRET)
    monkeypatch.setenv(_TMDB_BASE_URL_ENV, _SYNTHETIC_TMDB_URL)
    ...
    assert response.status_code == HTTPStatus.OK
    assert service.calls == 1
    for value in (_SYNTHETIC_TMDB_URL, _SYNTHETIC_TMDB_TOKEN, _SYNTHETIC_SECRET):
        assert value not in response.text
        assert value not in caplog.text


def test_failed_submission_does_not_affect_other_views() -> None:
    """Keep the other operational pages available after a failed refresh."""
    service = FakeAvailabilityService(_failure_report(WorkflowFailureReason.TRANSIENT_FAILURE))
    client = _client(service)
    token = _csrf_token(client)
    ...
    netflix_import = client.get("/imports/netflix")
    jellyfin_sync = client.get("/synchronizations/jellyfin")
    ...
    assert netflix_import.status_code == HTTPStatus.OK
    assert jellyfin_sync.status_code == HTTPStatus.OK
...
@pytest.mark.parametrize(
    ...
    [
        (
            [ProviderStatus(provider=ProviderKind.TMDB, configuration=ConfigurationState.CONFIGURED)],
            AvailabilityConfigurationState.CONFIGURED,
        ),
        (
            [
                ProviderStatus(
            ...
            AvailabilityConfigurationState.INVALID,
        ),
        (
            [ProviderStatus(provider=ProviderKind.TMDB, configuration=ConfigurationState.NOT_CONFIGURED)],
            AvailabilityConfigurationState.ABSENT,
        ),
        ([], AvailabilityConfigurationState.ABSENT),
    ],
)
def test_configuration_state_mapping_uses_safe_provider_status(
    ...
    expected: AvailabilityConfigurationState,
) -> None:
    """Map safe provider status snapshots to the availability configuration state."""
    assert availability_configuration_state(statuses) is expected


@pytest.mark.parametrize(
    ("configuration", "message"),
    [
        (AvailabilityConfigurationState.ABSENT, "not configured"),
        (AvailabilityConfigurationState.INVALID, "configuration is invalid"),
    ],
)
def test_configuration_gate_without_service_returns_safe_outcome(
    ...
    """Return the distinct configuration outcome when no refresh service is installed."""
...
@pytest.mark.parametrize("configuration", [None, AvailabilityConfigurationState.CONFIGURED])
def test_csrf_rejection_without_service_is_enforced(
    configuration: AvailabilityConfigurationState | None,
) -> None:
    """Enforce the established CSRF/origin rejection when no refresh service is installed."""
    client = _client_without_service(configuration=configuration)
    token = _csrf_token(client)
